...
+ (CAAnimation *)reverseAnimation:(CAAnimation *)anim totalDuration:(CGFloat)totalDuration {
    ...
    void (^reverseTimingFunction)(CAAnimation *) = ^(CAAnimation *theAnim) {
        ...
        if (timingFunction) {
            ...
            theAnim.timingFunction =
            [CAMediaTimingFunction functionWithControlPoints:1 - second[0]:1 - second[1]:1 - first[0]:1 - first[1]];
        }
    };
...
+ (CAAnimationGroup *)groupAnimations:(NSArray *)animations
                       ...
                       forEffectLayer:(BOOL)forEffectLayer
                       sublayersCount:(NSInteger)count {
...
+ (CGFloat)maxDurationFromAnimations:(NSArray *)anims {
    ...
    for (CAAnimation *anim in anims) {
        maxDuration =
        MAX(anim.beginTime + anim.duration * (CGFloat)(anim.repeatCount == 0 ? 1.0f : anim.repeatCount) * (anim.autoreverses ? 2.0f : 1.0f),
            maxDuration);
    }
...
+ (CGFloat)maxDurationOfEffectAnimation:(CAAnimationGroup *)anim sublayersCount:(NSInteger)count {
    ...
    if ([anim isKindOfClass:[CAAnimationGroup class]]) {
        for (CABasicAnimation *subAnim in anim.animations) {
            ...
            CGFloat duration =
            subAnim.beginTime + (subAnim.autoreverses ? subAnim.duration : 0) + (delay + subAnim.duration + repeatCountDuration);
...
+ (void)addSublayersAnimationNeedReverse:(CAAnimation *)anim
                                  forKey:(NSString *)key
                                forLayer:(CALayer *)layer
                        reverseAnimation:(BOOL)reverse
    ...
    void (^setBeginTime)(CAAnimation *, NSInteger) = ^(CAAnimation *subAnim, NSInteger sublayerIdx) {
        ...
        switch (orderType) {
            case 0:
                subAnim.beginTime += sublayerIdx * instanceDelay;
                break;
            case 1:
                subAnim.beginTime += (sublayersCount - sublayerIdx - 1) * instanceDelay;
                break;
            case 2: {
                CGFloat middleIdx = sublayersCount / 2.0f;
                CGFloat begin = fabs((middleIdx - sublayerIdx)) * instanceDelay;
                subAnim.beginTime += begin;
                break;
            }
            case 3: {
                CGFloat middleIdx = sublayersCount / 2.0f;
    ...
    [sublayers enumerateObjectsWithOptions:0
                                usingBlock:^(CALayer *sublayer, NSUInteger idx, BOOL *stop) {
                                    if ([anim isKindOfClass:[CAAnimationGroup class]]) {
                                        CAAnimationGroup *groupAnim = (CAAnimationGroup *)anim.copy;
                                        NSMutableArray *newSubAnimations =
                                        [NSMutableArray arrayWithCapacity:groupAnim.animations.count];
